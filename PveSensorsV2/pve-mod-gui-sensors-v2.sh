...
PVE_MOD_JS_SOURCE_FILE="$SCRIPT_CWD/PveSensors/PveMod_pvemanagerlib.js"
...
PVE_SENSOR_INFO_SOURCE_FILE="$SCRIPT_CWD/PveSensors/PveMod_SensorInfo.pm"
GPU_RRD_DIR="/var/lib/rrdcached/db/pve-mod-gpu"

# New modular architecture paths
PVEMOD_SOURCES_DIR="$SCRIPT_CWD/PveSensors"
PVEMOD_INSTALL_DIR="/usr/share/perl5/PVE/PVEMod"
PVEMOD_COLLECTOR_DIR="$PVEMOD_INSTALL_DIR/Collector"
PVEMOD_CONFIG_FILE="$PVEMOD_INSTALL_DIR/Config.pm"
...
install_sensor_monitor_module() {
    ...
    # Check if API facade source file exists
    if [[ ! -f "$PVE_SENSOR_INFO_SOURCE_FILE" ]]; then
        err "Source file not found: $PVE_SENSOR_INFO_SOURCE_FILE"
    fi

    # Create PVEMod module directories
    mkdir -p "$PVEMOD_INSTALL_DIR" || err "Failed to create directory: $PVEMOD_INSTALL_DIR"
    mkdir -p "$PVEMOD_COLLECTOR_DIR" || err "Failed to create directory: $PVEMOD_COLLECTOR_DIR"
    info "PVEMod module directories ready."

    # Copy the API facade module
    ...
    info "Copied API facade to $PVE_SENSOR_INFO_MOD_FILE"

    # Copy the new modular support files
    local -a module_files=(
        "PVEMod_Config.pm:$PVEMOD_INSTALL_DIR/Config.pm"
        "PVEMod_Utils.pm:$PVEMOD_INSTALL_DIR/Utils.pm"
        "PVEMod_Store.pm:$PVEMOD_INSTALL_DIR/Store.pm"
        "PVEMod_ProcessManager.pm:$PVEMOD_INSTALL_DIR/ProcessManager.pm"
        "PVEMod_Collectors/Intel.pm:$PVEMOD_COLLECTOR_DIR/Intel.pm"
        "PVEMod_Collectors/Nvidia.pm:$PVEMOD_COLLECTOR_DIR/Nvidia.pm"
    ...
    for entry in "${module_files[@]}"; do
        ...
        local dst="${entry##*:}"
        if [[ ! -f "$src" ]]; then
            err "Module source file not found: $src"
        fi
        cp "$src" "$dst" || err "Failed to copy $src to $dst"
        info "Copied $(basename "$src") to $dst"
    done
    ...
    # Patch configuration values in Config.pm (the canonical config location)
    sed -i "
    ...
    if [[ $? -eq 0 ]]; then
        ...
        warn "Failed to patch Config.pm settings."
    fi
}
...
function uninstall_mod {
	...
	# Remove PVEMod support modules (Config, Utils, Store, ProcessManager, Collectors)
	if [[ -d "$PVEMOD_INSTALL_DIR" ]]; then
		msgb "Removing PVEMod module directory: $PVEMOD_INSTALL_DIR"
		rm -rf "$PVEMOD_INSTALL_DIR"
		info "Removed PVEMod module directory."
	fi
