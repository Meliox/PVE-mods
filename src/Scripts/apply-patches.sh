...
_dry_forward() { patch -p1 -F0 -d "$PVE_MOD_ROOT" -f --dry-run -s < "$1" >/dev/null 2>&1; }
_dry_reverse() { patch -R -p1 -F0 -d "$PVE_MOD_ROOT" -f --dry-run -s < "$1" >/dev/null 2>&1; }
is_applied()   { _dry_reverse "$1"; }
can_apply()    { _dry_forward "$1"; }
do_apply()     { patch -p1 -F0 -d "$PVE_MOD_ROOT" -f -s < "$1"; }
do_revert()    { patch -R -p1 -F0 -d "$PVE_MOD_ROOT" -f -s < "$1"; }
